...
-- Define TweenService globally for use throughout
local SophiaMailTweenService = TweenService

local UI_SETTINGS_FOLDER = "SophiaMailer"

local SOPHIA_MAIL_DISCORD_LINK =
    "https://discord.gg/9n9xYBVBP"

-- Add new access keys here (one per line).
local SOPHIA_MAIL_VALID_KEYS = {
    "SophiaMail-2026",
    "Sophia-VIP",
}
...
local function SophiaMailNormalizeAccessKey(value)
    return tostring(
        value or ""
    )
        :gsub(
...
local function SophiaMailIsValidAccessKey(value)
    local normalized =
        SophiaMailNormalizeAccessKey(
            value
        )
    ...
    for _, validKey in ipairs(
        SOPHIA_MAIL_VALID_KEYS
    ) do
        if SophiaMailNormalizeAccessKey(
            validKey
        ) == normalized then
            return true
        end
...
local function SophiaMailOpenDiscordLink()
    ...
    pcall(function()
        if type(syn) == "table"
            and type(syn.open_url) == "function"
        then
            syn.open_url(link)
            opened =
                true
        elseif type(fluxus) == "table"
            and type(fluxus.open_url) == "function"
        then
            fluxus.open_url(link)
            opened =
                true
        elseif type(os) == "table"
    ...
    if opened ~= true then
        ...
        SophiaNotify(
            "Sophia Mail",
            "Successfully Copied Link Discord",
            4
        )
    end
...
function SophiaMailCreateKeyGate(onSuccess)
    ...
    textLabel(
        header,
        "KEY SYSTEM Welcome to The, Sophia Mail",
        UDim2.new(
            1,
            -52,
            1,
    ...
    local function submitKey()
        ...
        task.delay(
            0.12,
            function()
                if typeof(keyInput) == "Instance"
                    and keyInput.Parent ~= nil
                then
                    TweenService:Create(
                        keyInput,
                        TweenInfo.new(0.12),
    ...
    discordLink.MouseButton1Click:Connect(
        SophiaMailOpenDiscordLink
    )
...
function SophiaMailShowKeyGate()
    return SophiaMailCreateKeyGate(function()
        SophiaMailSetEnabled(true)
    end)
end
...
local SOPHIA_MAIL_COOLDOWN = 10.05
...
function SophiaMailCreateHud()
    ...
    footerDiscordLink.MouseEnter:Connect(function()
        TweenService:Create(
            footerDiscordLink,
            TweenInfo.new(0.08),
            {
    ...
    footerDiscordLink.MouseLeave:Connect(function()
        TweenService:Create(
            footerDiscordLink,
            TweenInfo.new(0.08),
            {
    ...
    footerDiscordLink.MouseButton1Click:Connect(
        SophiaMailOpenDiscordLink
    )
